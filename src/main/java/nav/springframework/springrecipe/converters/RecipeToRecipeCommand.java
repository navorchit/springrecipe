...
public class RecipeToRecipeCommand implements Converter<Recipe, RecipeCommand> {
    ...
    public RecipeCommand convert(Recipe source) {
        ...
        if (source.getCategories() != null && source.getCategories().size() > 0) source.getCategories()
                .forEach(category -> recipeCommand.getCategories().add(categoryConverter.convert(category)));
        if (source.getIngredients() != null && source.getIngredients().size() > 0) source.getIngredients()
